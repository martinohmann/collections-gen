...
// BarCollection is a collection of *equalityfunc.Foo values.
type BarCollection struct {
	items []*equalityfunc.Foo
}

// NewBarCollection creates a new collection from a slice of *equalityfunc.Foo.
func NewBarCollection(items []*equalityfunc.Foo) *BarCollection {
	return &BarCollection{items}
}

// Interface returns the underlying slice used by the collection as interface{}
// value.
func (c *BarCollection) Interface() interface{} {
	return c.items
}

// Items returns the underlying slice of *equalityfunc.Foo values used by the
// collection.
func (c *BarCollection) Items() []*equalityfunc.Foo {
	return c.items
}

// EachIndex calls fn for every item in the collection. The slice index of the
// item is passed to fn as the second argument.
func (c *BarCollection) EachIndex(fn func(*equalityfunc.Foo, int)) {
...
func (c *BarCollection) Each(fn func(*equalityfunc.Foo)) {
...
func (c *BarCollection) IndexOf(el *equalityfunc.Foo) int {
	for i, item := range c.items {
		if equalityfunc.Equal(item, el) {
			return i
		}
	}
...
func (c *BarCollection) First() *equalityfunc.Foo {
	return c.Nth(0)
}
...
func (c *BarCollection) FirstN(n int) []*equalityfunc.Foo {
	if n > c.Len() {
		return c.Items()
	}
...
// false and returns c.
func (c *BarCollection) Collect(fn func(*equalityfunc.Foo) bool) *BarCollection {
	return c.Filter(fn)
}

// Reject removes all items from the collection for which fn evaluates to
// true and returns c.
func (c *BarCollection) Reject(fn func(*equalityfunc.Foo) bool) *BarCollection {
...
func (c *BarCollection) Partition(fn func(*equalityfunc.Foo) bool) (*BarCollection, *BarCollection) {
	...
	return NewBarCollection(lhs), NewBarCollection(rhs)
}

// Map calls fn for each item in the collection an replaces its value with the
// result of fn.
func (c *BarCollection) Map(fn func(*equalityfunc.Foo) *equalityfunc.Foo) *BarCollection {
...
// result of fn.
func (c *BarCollection) MapIndex(fn func(*equalityfunc.Foo, int) *equalityfunc.Foo) *BarCollection {
	for i, item := range c.items {
		c.items[i] = fn(item, i)
...
func (c *BarCollection) Reduce(fn func(reducer *equalityfunc.Foo, item *equalityfunc.Foo) *equalityfunc.Foo) *equalityfunc.Foo {
...
func (c *BarCollection) Find(fn func(*equalityfunc.Foo) bool) *equalityfunc.Foo {
	item, _ := c.FindOk(fn)
...
func (c *BarCollection) FindOk(fn func(*equalityfunc.Foo) bool) (*equalityfunc.Foo, bool) {
	for _, item := range c.items {
		if fn(item) {
			return item, true
		}
...
func (c *BarCollection) Any(fn func(*equalityfunc.Foo) bool) bool {
	for _, item := range c.items {
		if fn(item) {
			return true
		}
...
func (c *BarCollection) All(fn func(*equalityfunc.Foo) bool) bool {
	for _, item := range c.items {
		if !fn(item) {
			return false
		}
...
func (c *BarCollection) Contains(el *equalityfunc.Foo) bool {
	for _, item := range c.items {
		if equalityfunc.Equal(item, el) {
			return true
		}
...
func (c *BarCollection) Sort(fn func(*equalityfunc.Foo, *equalityfunc.Foo) bool) *BarCollection {
...
func (c *BarCollection) IsSorted(fn func(*equalityfunc.Foo, *equalityfunc.Foo) bool) bool {
	return sort.SliceIsSorted(c.items, c.lessFunc(fn))
}

func (c *BarCollection) lessFunc(fn func(*equalityfunc.Foo, *equalityfunc.Foo) bool) func(int, int) bool {
...
func (c *BarCollection) Reverse() *BarCollection {
...
// of bounds.
func (c *BarCollection) Remove(pos int) *BarCollection {
	c.items = append(c.items[:pos], c.items[pos+1:]...)
	return c
}

// RemoveItem removes all instances of item from the collection and returns it.
func (c *BarCollection) RemoveItem(item *equalityfunc.Foo) *BarCollection {
	for i, el := range c.items {
		if equalityfunc.Equal(el, item) {
...
func (c *BarCollection) InsertItem(item *equalityfunc.Foo, pos int) *BarCollection {
	var zeroValue *equalityfunc.Foo
...
// underlying slice.
func (c *BarCollection) Cut(i, j int) []*equalityfunc.Foo {
...
func (c *BarCollection) Slice(i, j int) []*equalityfunc.Foo {
	return c.items[i:j]
}
